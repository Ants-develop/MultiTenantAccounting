...
export const JobsKanban = ({ stages, filters, onJobClick }: JobsKanbanProps) => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const job = jobs?.find((j: WorkflowWithDetails) => j.id === jobId);
  ...
  const getJobsByStage = (stageId: string) => {
    return jobs?.filter((job: WorkflowWithDetails) => job.current_stage_id === stageId) || [];
  };

  const activeJob = jobs?.find((job: WorkflowWithDetails) => job.id === activeId);
        ...
        {stages.map((stage) => {
          ...
          return (
            <SortableContext
              key={stage.id}
              id={stage.id}
              items={stageJobs.map((job: WorkflowWithDetails) => job.id)}
                  ...
                  <ScrollArea className="h-[calc(100vh-16rem)]">
                    <div className="space-y-3 pr-4">
                      {stageJobs.length === 0 ? (
                        ...
                        stageJobs.map((job: WorkflowWithDetails) => (
                          <SortableJobCard
                            key={job.id}
                            job={job}
