...
(async () => {
  ...
  // Start template scheduler worker
  const { templateSchedulerWorker } = await import("./workers/template-scheduler-worker");
  templateSchedulerWorker.start();
  ...
  if (app.get("env") === "development" && process.env.VITE_CLI_MODE !== "true") {
