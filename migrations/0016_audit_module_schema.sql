...
CREATE TABLE IF NOT EXISTS audit."1690_stock" (
    ...
    company_code UUID REFERENCES public.clients(id) ON DELETE CASCADE,
...
ALTER TABLE audit."1690_stock" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."1690_stock"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."1690_stock".company_code
...
ALTER TABLE audit."accounts_summary" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."accounts_summary"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."accounts_summary".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."accrued_interest" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."accrued_interest"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."accrued_interest".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."analytics" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."analytics"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."analytics".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."analytics_balance_summary" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."analytics_balance_summary"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."analytics_balance_summary".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."capital_accounts" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."capital_accounts"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."capital_accounts".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."capital_accounts_summary" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."capital_accounts_summary"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."capital_accounts_summary".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."creditors_avans" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."creditors_avans"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."creditors_avans".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."debitors_avans" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."debitors_avans"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."debitors_avans".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."dublicate_creditors" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."dublicate_creditors"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."dublicate_creditors".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."dublicate_debitors" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."dublicate_debitors"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."dublicate_debitors".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."high_amount_per_quantity_summary" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."high_amount_per_quantity_summary"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."high_amount_per_quantity_summary".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."negativ_creditor" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."negativ_creditor"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."negativ_creditor".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."negativ_debitor" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."negativ_debitor"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."negativ_debitor".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."negative_balance_141_summary" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."negative_balance_141_summary"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."negative_balance_141_summary".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."negative_balance_311_summary" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."negative_balance_311_summary"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."negative_balance_311_summary".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."negative_balance_summary" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."negative_balance_summary"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."negative_balance_summary".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."negative_loans" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."negative_loans"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."negative_loans".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."negative_stock" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."negative_stock"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."negative_stock".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."negativ_interest" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."negativ_interest"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."negativ_interest".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."negativ_salary" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."negativ_salary"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."negativ_salary".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."positive_balance_summary" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."positive_balance_summary"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."positive_balance_summary".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."revaluation_status_summary" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."revaluation_status_summary"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."revaluation_status_summary".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."salary_expense" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."salary_expense"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."salary_expense".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
...
ALTER TABLE audit."writeoff_stock" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own company audit data" ON audit."writeoff_stock"
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.user_companies
            WHERE user_companies.user_id = auth.uid()
            AND user_companies.client_id = audit."writeoff_stock".company_code
            AND user_companies.is_active = true
        )
        OR
        EXISTS (
