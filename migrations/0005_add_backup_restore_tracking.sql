...
CREATE TABLE IF NOT EXISTS backup_restore_history (
    ...
    file_hash TEXT,
    storage_source TEXT NOT NULL DEFAULT 'google_drive' CHECK (storage_source IN ('google_drive', 'supabase_storage')),
    temp_database_name TEXT,
    restore_status TEXT NOT NULL DEFAULT 'pending' CHECK (restore_status IN ('pending', 'downloading', 'restoring', 'migrating', 'completed', 'failed')),
