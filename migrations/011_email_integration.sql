-- =====================================================
-- Migration 011: Email Integration (Gmail API)
-- =====================================================
-- This migration creates tables for email integration:
-- - Email accounts (Gmail OAuth tokens)
-- - Email messages (inbox storage)
-- - Email templates
-- - Email routing rules

-- UP
-- =====================================================
-- Table 1: email_accounts
-- Email account configurations
-- =====================================================
CREATE TABLE IF NOT EXISTS email_accounts (
    id SERIAL PRIMARY KEY,
    user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
    client_id INTEGER REFERENCES clients(id) ON DELETE SET NULL, -- Optional: route emails to specific client
...
COMMENT ON TABLE email_accounts IS 'Email account configurations for Gmail API integration';
COMMENT ON COLUMN email_accounts.access_token IS 'Encrypted OAuth access token';
COMMENT ON COLUMN email_accounts.refresh_token IS 'Encrypted OAuth refresh token';
COMMENT ON COLUMN email_accounts.imap_password IS 'Legacy field - can store encrypted token as fallback';
COMMENT ON COLUMN email_accounts.smtp_password IS 'Legacy field - can store encrypted refresh token as fallback';

-- =====================================================
-- Table 2: email_messages
-- Email messages storage
-- =====================================================
CREATE TABLE IF NOT EXISTS email_messages (
    id SERIAL PRIMARY KEY,
    email_account_id INTEGER NOT NULL REFERENCES email_accounts(id) ON DELETE CASCADE,
    client_id INTEGER REFERENCES clients(id) ON DELETE SET NULL, -- If routed to client
...
COMMENT ON TABLE email_messages IS 'Email messages storage from Gmail API';
COMMENT ON COLUMN email_messages.attachments IS 'JSON array of attachment metadata: {name, size, contentType, url}';
...
COMMENT ON TABLE email_templates IS 'Email templates for sending automated emails';
COMMENT ON COLUMN email_templates.variables IS 'JSON object describing available template variables';

-- =====================================================
-- Table 4: email_routing_rules
-- Email routing rules
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_email_routing_rules_is_active ON email_routing_rules(is_active);
CREATE INDEX IF NOT EXISTS idx_email_routing_rules_priority ON email_routing_rules(priority);

COMMENT ON TABLE email_routing_rules IS 'Email routing rules for automatic email processing';
COMMENT ON COLUMN email_routing_rules.condition IS 'JSON object with rule condition: {type, value, operator}';
COMMENT ON COLUMN email_routing_rules.action_config IS 'JSON object with action-specific configuration';
...
CREATE TRIGGER email_routing_rules_updated_at BEFORE UPDATE ON email_routing_rules
    FOR EACH ROW EXECUTE FUNCTION update_email_updated_at();
...
-- Drop triggers first
-- =====================================================
DROP TRIGGER IF EXISTS email_routing_rules_updated_at ON email_routing_rules;
...
-- =====================================================
DROP TABLE IF EXISTS email_routing_rules;
